...
class Api::V1::ProductsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should update product' do
    put api_v1_product_url(@product), params: {product: {title: @product.title}},
        headers: {Authorization: JsonWebToken.encode(user_id: @product.user_id)}, as: :json
    assert_response :success
  end

  test 'should forbid update if user != product.user' do
    put api_v1_product_url(@product), params: {product: {title: @product.title}},
        headers: {Authorization: JsonWebToken.encode(user_id: users(:two).id)}, as: :json
    assert_response :forbidden
  end

  test 'should forbid update' do
    put api_v1_product_url(@product), params: {product: {title: @product.title}}, as: :json
    assert_response :forbidden
  end
